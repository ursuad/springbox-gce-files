...
header "Run the commands below in separate terminal tabs, because they are blocking"
echo "kubectl port-forward -p ${AUTH_SERVER_POD_NAME} ${AUTH_SERVER_POD_PORT}"
echo "kubectl port-forward -p ${REVIEWS_POD_NAME} ${REVIEWS_POD_PORT}"
echo "kubectl port-forward -p ${RECOMMENDATIONS_POD_NAME} ${RECOMMENDATIONS_POD_PORT}"
